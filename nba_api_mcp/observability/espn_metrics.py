...
class ESPNMetricsTracker:
    ...
    def get_metrics(self) -> ESPNMetrics:
        ...
        last_24h_success_rate = (
            (last_24h_successful / last_24h_total * 100) if last_24h_total > 0 else 0.0
        )
        ...
        odds_coverage = (total_odds / total_games * 100) if total_games > 0 else 0.0
    ...
    def detect_schema_drift(
        self, expected_fields: List[str], actual_data: dict
    ) -> List[str]:
        """
...
if __name__ == "__main__":
    ...
    tracker.record_call(
        success=True, response_time_ms=150, games_fetched=10, odds_found=8
    )
    tracker.record_call(
        success=True, response_time_ms=200, games_fetched=12, odds_found=12
    )
    tracker.record_call(success=False, response_time_ms=5000, error_message="Timeout")
    tracker.record_call(
        success=True, response_time_ms=180, games_fetched=8, odds_found=6
    )

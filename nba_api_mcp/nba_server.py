...
_project_root = Path(
    __file__
).parent.parent  # nba_mcp/nba_mcp/nba_server.py -> nba_mcp/
...
from nba_api_mcp.api.advanced_metrics_calculator import AdvancedMetricsCalculator
from nba_api_mcp.api.client import NBAApiClient
from nba_api_mcp.api.entity_resolver import (
    get_cache_info,
    resolve_entity,
    suggest_players,
    suggest_teams,
...
from nba_api_mcp.api.game_context import get_game_context as fetch_game_context
from nba_api_mcp.api.lineup_tracker import add_lineups_to_play_by_play

# Import new response models and error handling
from nba_api_mcp.api.models import (
    EntityReference,
    PlayerComparison,
    PlayerSeasonStats,
    ResponseEnvelope,
...
from nba_api_mcp.api.schedule import format_schedule_markdown
from nba_api_mcp.api.schedule import get_nba_schedule as fetch_nba_schedule

# Import data groupings and advanced metrics (Phase 4)
from nba_api_mcp.api.season_aggregator import (
    get_player_season_stats,
    get_team_season_stats,
)

# Import season context for LLM temporal awareness
from nba_api_mcp.api.season_context import get_current_season, get_season_context
from nba_api_mcp.api.shot_charts import get_shot_chart as fetch_shot_chart

# Import date parser for natural language date support
from nba_api_mcp.api.tools.date_parser import parse_and_normalize_date_params
from nba_api_mcp.api.tools.nba_api_utils import (
    format_game,
    get_player_id,
    get_player_name,
...
from nba_api_mcp.cache.redis_cache import CacheTier, cached, get_cache, initialize_cache

# Import dataset and joins features
from nba_api_mcp.data.catalog import get_catalog
from nba_api_mcp.data.dataset_manager import get_manager as get_dataset_manager
from nba_api_mcp.data.dataset_manager import initialize_manager, shutdown_manager
from nba_api_mcp.data.fetch import fetch_endpoint, validate_parameters
from nba_api_mcp.data.joins import filter_table, join_tables, join_with_stats

# Import NLQ pipeline components
from nba_api_mcp.nlq.pipeline import answer_nba_question as nlq_answer_question
from nba_api_mcp.nlq.pipeline import get_pipeline_status
from nba_api_mcp.nlq.tool_registry import initialize_tool_registry

# Import Week 4 observability (metrics + tracing)
from nba_api_mcp.observability import (
    get_metrics_manager,
    get_tracing_manager,
    initialize_metrics,
    initialize_tracing,
...
async def get_league_leaders_info(
    ...
    try:
        ...
        if conference is not None:
            if "TEAM" in df.columns:
                df = df[
                    df["TEAM"].map(lambda t: TEAM_TO_CONFERENCE.get(t) == conference)
                ]
        ...
        if format == "json":
            ...
            for _, r in df.head(limit).iterrows():
                leader = {
                    "player_name": str(
                        r.get("PLAYER_NAME", r.get("PLAYER", "Unknown"))
                    ),
                    "team": str(r.get("TEAM", "N/A")),
                    "value": (
                        float(r.get(stat_category, 0))
                        if pd.notna(r.get(stat_category))
                        else None
                    ),
                }
...
async def get_all_time_leaders(
    ...
    try:
        ...
        all_time_data = await asyncio.to_thread(alltimeleadersgrids.AllTimeLeadersGrids)
        ...
        if format == "json":
            ...
            for _, row in df.iterrows():
                leader = {
                    ...
                    "value": (
                        float(row.get(stat_upper, 0))
                        if pd.notna(row.get(stat_upper))
                        else None
                    ),
                    "rank": (
                        int(row.get(rank_col, 0)) if rank_col in df.columns else None
                    ),
...
async def get_live_scores(target_date: Optional[str] = None, **kwargs) -> str:
    ...
    if target_date:
        logger.info(
            f"[get_live_scores] Querying historical scores for date: {target_date}"
        )
    else:
        logger.info(
            f"[get_live_scores] Querying live scores (no date specified, using today)"
        )
...
def format_game_log(
    df: pd.DataFrame, team: Optional[str] = None, season: Optional[str] = None
) -> str:
    """
    ...
    # Column headers
    lines.append(
        f"{'Date':<12} {'Matchup':<20} {'W/L':<4} {'PTS':<4} {'OPP_PTS':<8} {'+/-':<6}"
    )
    lines.append("-" * 60)
    ...
    for _, row in df.iterrows():
        try:
            ...
            lines.append(
                f"{'ERROR':<12} {'Error formatting game':<20} {'N/A':<4} {'N/A':<4} {'N/A':<8} {'N/A':<6}"
            )
...
async def get_date_range_game_log_or_team_game_log(
    ...
    try:
        ...
        logger.info(
            f"Returning {len(games)} games as JSON (execution: {execution_time_ms:.1f}ms)"
        )
        return response.to_json_string()
...
async def fetch_player_games(
    ...
    player: Optional[
        Union[int, str]
    ] = None,  # Player ID (2544) or name ("LeBron James")
    team: Optional[
        Union[int, str]
    ] = None,  # Team ID (1610612747) or name ("Lakers" or "LAL")
    date_from: Optional[str] = None,
    date_to: Optional[str] = None,
    ...
    try:
        ...
        from nba_api_mcp.utils.entity_utils import (
        ...
        logger.info(
            f"fetch_player_games: Fetching {len(seasons)} seasons with filters: {filters}"
        )
        df = await fetch_grouping_multi_season(
            "player/game", seasons=seasons, **filters
        )
        ...
        return json.dumps(
            {"status": "error", "error_code": "FETCH_ERROR", "error_message": str(e)},
            indent=2,
        )
...
async def play_by_play(
    ...
    if include_lineups:
        try:
            ...
            from nba_api_mcp.api.tools.playbyplayv3_or_realtime import (
                PastGamesPlaybyPlay,
            )

            # Get game_id from date/team
            pbp_instance = PastGamesPlaybyPlay.from_team_date(
                when=game_date, team=team, show_choices=False
            )
...
async def get_lineup_stats(
    ...
    try:
        ...
        lineups_df = _fetch_lineup_data_cached(
            team_id=team_id, season=season, season_type="Regular Season"
        )
            ...
            def player_in_lineup(lineup_name, player_search):
                ...
                for lineup_player in lineup_players:
                    similarity = SequenceMatcher(
                        None, lineup_player.lower(), player_lower
                    ).ratio()
...
async def get_lineup_stats_multi_season(
    team: str, seasons: str, min_minutes: int = 10, aggregation: str = "separate"
) -> str:
    """
    ...
    try:
        ...
        from nba_api_mcp.utils.season_utils import parse_season_input
        ...
        async def fetch_season_lineup(season_str):
            ...
            try:
                # Use cached fetcher
                lineups_df = _fetch_lineup_data_cached(
                    team_id=team_id, season=season_str, season_type="Regular Season"
                )
                ...
                if lineups_df.empty:
                    return (
                        season_str,
                        None,
                        f"No lineups with {min_minutes}+ minutes in {season_str}",
                    )
        ...
        if aggregation == "combined":
            ...
            grouped = (
                combined_df.groupby("GROUP_NAME")
                .agg(
                    {
                        "GP": "sum",
                        "W": "sum",
            ...
            # Format response
            from nba_api_mcp.utils.season_utils import format_season_display
            ...
            for season_str, lineups_df, error in season_results:
                ...
                for idx, row in lineups_df.head(
                    10
                ).iterrows():  # Show top 10 per season
                    lineup_name = row["GROUP_NAME"]
                    gp = int(row["GP"])
                    wins = int(row["W"])
                    losses = int(row["L"])
...
async def get_lineup_trends(
    ...
    try:
        ...
        lineup_stats = await get_lineup_stats(
            team=team, season=season, min_minutes=min_minutes
        )
    ...
    try:
        ...
        if entity_type == "player":
            ...
            stats = await get_player_season_stats(
                season=season, player_id=entity.entity_id, team_id=team_id
            )

            grouping_level = "player/team/season" if team_filter else "player/season"

        else:  # team
            stats = await get_team_season_stats(season=season, team_id=entity.entity_id)
        ...
        logger.debug(
            f"DEBUG - Season stats keys: {stats.keys() if isinstance(stats, dict) else 'Not dict'}"
        )
        logger.debug(
            f"DEBUG - Season stats sample (first 500 chars): {str(stats)[:500]}"
        )
...
async def get_advanced_metrics(
    player_name: str, season: str, metrics: Optional[List[str]] = None
) -> str:
    """
    ...
    try:
        ...
        if metrics:
            metrics_dict = {
                ...
                if k in metrics or k in ["PLAYER_NAME", "SEASON"]
            }
...
async def get_team_standings(
    ...
    try:
        from nba_api_mcp.api.advanced_stats import get_team_standings as fetch_standings
...
async def get_team_advanced_stats(team_name: str, season: Optional[str] = None) -> str:
    ...
    try:
        from nba_api_mcp.api.advanced_stats import (
            get_team_advanced_stats as fetch_stats,
        )
...
async def get_player_advanced_stats(
    ...
    try:
        from nba_api_mcp.api.advanced_stats import (
            get_player_advanced_stats as fetch_stats,
        )
...
async def compare_players(
    ...
    try:
        from nba_api_mcp.api.advanced_stats import compare_players as do_compare
...
async def compare_players_era_adjusted(
    ...
    try:
        from nba_api_mcp.api.era_adjusted import (
...
async def get_shot_chart(
    ...
    **PARAMETER FLEXIBILITY FOR SMALLER MODELS:**
    ...
    try:
        ...
        logger.info(
            f"[get_shot_chart] Called with params: entity_name={entity_name}, game_date={game_date}, date_from={date_from}, date_to={date_to}"
        )

        # Convert game_date to date_from/date_to if provided
        if game_date is not None:
            logger.info(
                f"[get_shot_chart] Converting game_date parameter: {game_date} → date_from={game_date}, date_to={game_date}"
            )
            date_from = game_date
            date_to = game_date

        logger.info(
            f"[get_shot_chart] Final date parameters: date_from={date_from}, date_to={date_to}"
        )
...
async def get_metrics_info() -> str:
    ...
    """
    try:
        from nba_api_mcp.observability import get_metrics_snapshot
...
async def list_endpoints(category: Optional[str] = None) -> str:
    ...
    try:
        ...
        from nba_api_mcp.data.catalog import EndpointCategory
...
async def catalog() -> str:
    ...
    try:
        ...
        for cat_name, endpoints in sorted(endpoints_by_cat.items()):
            ...
            for endpoint in endpoints:
                lines.append(f"- **{endpoint['display_name']}** (`{endpoint['name']}`)")
...
async def fetch(endpoint: str, params: Dict[str, Any]) -> str:
    ...
    try:
        ...
        from nba_api_mcp.data.introspection import get_introspector
        ...
        handle = await manager.store(
            table,
            name=f"{endpoint}_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
            provenance=provenance,
        )

        # Import limits helper for display
        from nba_api_mcp.data.limits import get_limits
        ...
        lines.append(
            f"Use this handle for joins: `join(['{handle.uuid}', ...], on=...)`"
        )
        lines.append(f"Or save to mcp_data/: `save_dataset('{handle.uuid}')`")
        lines.append(
            f"Or save custom path: `save_dataset('{handle.uuid}', 'path/to/file.parquet')`"
        )
...
async def join(
    ...
    try:
        ...
        # Store joined result
        from nba_api_mcp.data.dataset_manager import ProvenanceInfo
...
async def save_nba_data(
    data_json: str, custom_filename: Optional[str] = None, format: str = "auto"
) -> str:
    """
    ...
    try:
        ...
        if custom_filename:
            ...
            from nba_api_mcp.data.dataset_manager import generate_descriptive_filename
...
async def save_dataset(
    ...
    try:
        ...
        if path is None:
            from nba_api_mcp.data.dataset_manager import get_default_save_path
...
async def fetch_chunked(
    ...
    try:
        from nba_api_mcp.data.introspection import get_introspector
        from nba_api_mcp.data.pagination import get_paginator
        ...
        # Build the handles list string without nested f-strings
        handle_items = [f'"{c["handle"]}"' for c in chunks[:3]]
        handles_str = (
            f"handles = [{', '.join(handle_items)}{'...' if len(chunks) > 3 else ''}]"
        )
    ...
    try:
        ...
        for category, eps in sorted(by_category.items()):
            ...
            for ep in eps:
                ...
                if required_params:
                    lines.append(
                        f"- **Required Parameters**: {', '.join([f'`{p.name}`' for p in required_params])}"
                    )

                if optional_params:
                    lines.append(
                        f"- **Optional Parameters**: {', '.join([f'`{p.name}`' for p in optional_params])}"
                    )
    ...
    try:
        ...
        if max_fetch_mb is not None:
            ...
            if max_fetch_mb == -1:
                ...
                action_msg = (
                    f"✓ Limit updated: {old_limit:.0f} MB → {max_fetch_mb:.0f} MB"
                )
                warning_msg = ""
        else:
...
async def get_player_game_stats(
    ...
    try:
        ...
        if opponent:
            ...
            try:
                from nba_api_mcp.api.entity_resolver import resolve_entity
            ...
            if opponent_team_name and "MATCHUP" in df.columns:
                df = df[
                    df["MATCHUP"].str.contains(opponent_team_name, case=False, na=False)
                ]

        if game_date:
            # Normalize game date
            try:
                from nba_api_mcp.api.tools.nba_api_utils import normalize_date
        ...
        if len(df) > 1:
            ...
            response_lines.append(
                f"**Averages**: {avg_pts:.1f} PTS, {avg_reb:.1f} REB, {avg_ast:.1f} AST"
            )
            response_lines.append("")
            response_lines.append("---")
            response_lines.append("")
        ...
        for idx, row in df.iterrows():
            ...
            fga = row.get("FGA", 0)
            fg_pct = (
                row.get("FG_PCT", 0) * 100
                if "FG_PCT" in row and pd.notna(row["FG_PCT"])
                else 0
            )
            fg3m = row.get("FG3M", 0)
            fg3a = row.get("FG3A", 0)
            fg3_pct = (
                row.get("FG3_PCT", 0) * 100
                if "FG3_PCT" in row and pd.notna(row["FG3_PCT"])
                else 0
            )
            ...
            response_lines.append(
                f"**Stats**: {pts} PTS, {reb} REB, {ast} AST, {stl} STL, {blk} BLK"
            )
            response_lines.append(
                f"**Shooting**: {fgm}/{fga} FG ({fg_pct:.1f}%), {fg3m}/{fg3a} 3PT ({fg3_pct:.1f}%)"
            )
...
async def get_box_score(
    ...
    """
    logger.debug(
        f"get_box_score(game_id={game_id}, team={team}, game_date={game_date})"
    )
    ...
    try:
        ...
        if game_date:
            ...
            normalized_params, debug_messages = parse_and_normalize_date_params(
                params, date_param_name="game_date", date_param_aliases=["date", "day"]
            )
...
"""
            ...
            if isinstance(games_result, pd.DataFrame):
                for _, game in games_result.iterrows():
                    if (
                        ...
                        or game["visitor_team"]["id"] == team_id
                    ):
        ...
        if line_score is not None and not line_score.empty:
            ...
            for _, row in line_score.iterrows():
                ...
                response_lines.append(
                    f"**{team_abbrev}**: {quarters_str} | **TOTAL: {pts}**"
                )
        ...
        if team_stats is not None and not team_stats.empty:
            ...
            for _, row in team_stats.iterrows():
                ...
                tov = row.get("TOV", 0)
                fg_pct = (
                    row.get("FG_PCT", 0) * 100 if pd.notna(row.get("FG_PCT")) else 0
                )
                fg3_pct = (
                    row.get("FG3_PCT", 0) * 100 if pd.notna(row.get("FG3_PCT")) else 0
                )

                response_lines.append(f"### {team_abbrev}")
                response_lines.append(
                    f"**Totals**: {pts} PTS, {reb} REB, {ast} AST, {stl} STL, {blk} BLK, {tov} TOV"
                )
                response_lines.append(
                    f"**Shooting**: {fg_pct:.1f}% FG, {fg3_pct:.1f}% 3PT"
                )
                response_lines.append("")
        ...
        for team_abbrev in teams:
            team_players = player_stats[
                player_stats["TEAM_ABBREVIATION"] == team_abbrev
            ]
...
async def get_clutch_stats(
    ...
    try:
        ...
        if entity_type == "player":
            ...
            response_lines.append(
                f"**Field Goals**: {fgm:.1f}/{fga:.1f} ({fg_pct:.1f}%)"
            )
            response_lines.append(
                f"**Three Pointers**: {fg3m:.1f}/{fg3a:.1f} ({fg3_pct:.1f}%)"
            )
            response_lines.append(
                f"**Free Throws**: {ftm:.1f}/{fta:.1f} ({ft_pct:.1f}%)"
            )
            response_lines.append("")
            ...
            response_lines.append(
                f"**Rebounds**: {reb:.1f} ({oreb:.1f} OFF, {dreb:.1f} DEF)"
            )
            ...
            response_lines.append(f"**Points**: {pts:.1f} {per_mode.lower()}")
            response_lines.append(
                f"**FG%**: {fg_pct:.1f}% | **3P%**: {fg3_pct:.1f}% | **FT%**: {ft_pct:.1f}%"
            )
            ...
            response_lines.append(
                f"**Assists**: {ast:.1f} | **Rebounds**: {reb:.1f} | **Turnovers**: {tov:.1f}"
            )
            response_lines.append("")
...
async def get_player_head_to_head(
    ...
    try:
        ...
        result = await client.get_player_head_to_head(
            player1_name=player1_name, player2_name=player2_name, season=season
        )
...
async def get_player_performance_splits(
    ...
    try:
        ...
        result = await client.get_player_performance_splits(
            player_name=player_name, season=season, last_n_games=last_n_games
        )
    ...
    try:
        ...
        if player_name:
            ...
            awards_df = await client.get_player_awards(
                player_name=player_name, award_filter=award_type  # Optional filter
            )
            ...
            # Get winners
            winners = client.get_award_winners(award_type=award_type, last_n=last_n)
...
def main():
    ...
    logger.info(
        f"NLQ tool registry initialized with {len(tool_map)} tools (8 original + 12 Phase 1)"
    )

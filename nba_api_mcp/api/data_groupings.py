...
class PlayerGameGrouping(DataGrouping):
    ...
    async def fetch(self, **filters) -> pd.DataFrame:
        ...
        """
        from nba_api_mcp.api.data_filtering import split_filters
        ...
        if has_player_filter:
            ...
            from nba_api_mcp.api.tools.nba_api_utils import get_player_name
        ...
        if use_direct_api and df is None:
            ...
            if unified_filters:
                ...
                from nba_api_mcp.data.unified_fetch import apply_filters
...
class TeamGameGrouping(DataGrouping):
    ...
    async def fetch(self, **filters) -> pd.DataFrame:
        ...
        if has_team_filter:
            ...
            from nba_api_mcp.api.tools.nba_api_utils import get_team_name
        ...
        if use_league_endpoint:
            ...
            try:
                result = await unified_fetch(
                    endpoint="league_team_games", params=params, filters=unified_filters
                )
...
class GroupingFactory:
    ...
    def create(cls, grouping_level: Union[GroupingLevel, str]) -> DataGrouping:
        ...
        if not grouping_class:
            raise ValueError(
                f"Grouping level {grouping_level.value} not yet implemented"
            )
    ...
    @classmethod
    def register(
        cls, grouping_level: GroupingLevel, grouping_class: type[DataGrouping]
    ):
...
async def fetch_grouping(
    ...
    **filters,
    ...
    if not grouping.validate_params(filters):
        ...
        raise ValueError(
            f"Missing required parameters for {grouping_level}: {required}"
        )
    ...
    if enrich and not df.empty:
        ...
        df = await enrich_dataset(
            ...
            use_defaults=(
                enrichments is None
            ),  # Use defaults if no specific enrichments requested
...
def merge_with_advanced_metrics(
    ...
    """
    from nba_api_mcp.data.merge_manager import MergeManager, MergeValidationLevel
    ...
    validation_enum = validation_map.get(
        validation_level.lower(), MergeValidationLevel.WARN
    )
...
def get_merge_identifier_columns(
    grouping_level: Union[GroupingLevel, str],
) -> Dict[str, List[str]]:
    """
    ...
    """
    from nba_api_mcp.data.merge_manager import get_merge_config
...
def list_all_merge_configs() -> Dict[str, Dict[str, Any]]:
    ...
    """
    from nba_api_mcp.data.merge_manager import list_merge_configs

...
from nba_api_mcp.api.tools.nba_api_utils import normalize_season
    ...
    try:
        ...
        def extract_team_standings(row: pd.Series) -> Dict[str, Any]:
            ...
            return {
                ...
                "conference": (
                    str(row["Conference"].iloc[0]) if "Conference" in row else ""
                ),
                "division": (str(row["Division"].iloc[0]) if "Division" in row else ""),
            }
...
async def fetch_advanced_stats_context(
    ...
    try:
        ...
        team1_name = next(
            (t["full_name"] for t in all_teams if t["id"] == team1_id), None
        )
        team2_name = next(
            (t["full_name"] for t in all_teams if t["id"] == team2_id), None
        )

        if not team1_name or not team2_name:
            logger.error(
                f"Could not resolve team names for IDs: {team1_id}, {team2_id}"
            )
            return {"team1": {}, "team2": {}}

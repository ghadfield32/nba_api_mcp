...
def parse_relative_date(date_string: Optional[str]) -> Tuple[Optional[str], str]:
    ...
    if not date_string or not isinstance(date_string, str):
        logger.debug(
            "[date_parser] No date provided, returning None (API will use today)"
        )
    ...
    if re.match(r"^\d{4}-\d{2}-\d{2}$", date_string):
        try:
            ...
            error_msg = (
                f"Invalid date format: {original_input} (not a valid calendar date)"
            )
...
def normalize_parameter_name(
    params: dict, old_name: str, new_name: str
...
def parse_and_normalize_date_params(
    params: dict, date_param_name: str = "target_date", date_param_aliases: list = None
) -> Tuple[dict, list]:
    """
...
def validate_date_parser():
    ...
    for input_val, expected in test_cases:
        ...
        if result != expected:
            ...
            logger.debug(f"[date_parser TEST PASSED] Input: {input_val} → {result}")

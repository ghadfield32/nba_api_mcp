...
from nba_api_mcp.observability.espn_metrics import track_espn_call
...
def _build_odds_key(
    home_abbr: Optional[str], away_abbr: Optional[str]
) -> Optional[str]:
    """
...
def _fetch_espn_scoreboard(date_label: Optional[str], timeout: int) -> Optional[dict]:
    ...
    try:
        ...
        Exception
    ) as exc:  # pragma: no cover - network failures are expected sometimes

...
class WinSharesCalculator:
    ...
    def calculate_offensive_win_shares(
        ...
        league_avg_per_min = (
            league_avg_ortg / 100 / 48
        )  # Points per minute at league avg
    ...
    def calculate_defensive_win_shares(
        player_stats: Dict[str, float], team_stats: Dict[str, float], season: str
    ) -> float:
        """
    ...
    def calculate_win_shares(
        ...
        ows = WinSharesCalculator.calculate_offensive_win_shares(
            player_stats, team_stats or {}, league_avg_ortg, season
        )

        dws = WinSharesCalculator.calculate_defensive_win_shares(
            player_stats, team_stats or {}, season
        )
...
class AdvancedMetricsCalculator:
    ...
    async def calculate_all_metrics(
        ...
        metrics = AdvancedMetrics(player_name=player_name, season=season)
    ...
    async def _fetch_player_season_stats(
        ...
        stats_dict = await get_player_season_stats(
            season=season, player_id=player_entity.entity_id
        )

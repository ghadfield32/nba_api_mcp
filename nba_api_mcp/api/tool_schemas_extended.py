...
class GetLeagueLeadersInfoInput(BaseModel):
    ...
    conference: Optional[str] = Field(
        None, description=("Filter by conference. 'East' or 'West'. " "Optional")
    )

    team: Optional[str] = Field(
        None,
        description=(
            "Filter by specific team. " "Examples: 'Lakers', 'LAL'. " "Optional"
        ),
        examples=Examples.TEAM_NAMES,
    )
...
class GetLiveScoresInput(BaseModel):
    ...
    class Config:
        json_schema_extra = {"examples": [{}, {"target_date": "2024-12-25"}]}
...
class GetNbaAwardsInput(BaseModel):
    ...
    format: str = Field(
        "text", description=("Output format. 'text' (default) or 'json'. " "Optional")
    )
...
class GetNbaScheduleInput(BaseModel):
    ...
    def validate_season(cls, v: Optional[str]) -> Optional[str]:
        ...
        if v is not None and "-" in v:
            ...
            if len(parts) != 2:
                raise ValueError(
                    f"Invalid season format: {v}. Expected 'YYYY-YY' (e.g., '2025-26')"
                )
        return v
    ...
    def validate_date(cls, v: Optional[str]) -> Optional[str]:
        ...
        if len(v) == 10 and v[4] == "-" and v[7] == "-":
            try:
                ...
                raise ValueError(
                    f"Invalid date format: {v}. Expected YYYY-MM-DD (e.g., '2025-12-01')"
                )
        else:
            raise ValueError(
                f"Invalid date format: {v}. Expected YYYY-MM-DD (e.g., '2025-12-01')"
...
class GetAdvancedMetricsInput(BaseModel):
    ...
    season: str = Field(
        ...,
        description=(
            "Season in YYYY-YY format. " "Examples: '2023-24', '2024-25'. " "REQUIRED"
        ),
        examples=Examples.SEASONS,
    )
        ...
        json_schema_extra = {
            "examples": [
                {"player_name": "LeBron James", "season": "2023-24"},
                {
                    ...
                    "season": "2023-24",
                    "metrics": ["WIN_SHARES", "GAME_SCORE_PER_36"],
                },
            ]
        }
    ...
    class Config:
        json_schema_extra = {
            "examples": [
                ...
                {
                    "player1_name": "Kobe Bryant",
                    "player2_name": "Luka Doncic",
                    "season1": "2005-06",
                    "season2": "2023-24",
                },
            ]
...
class PlayByPlayInput(BaseModel):
    ...
    recent_n: int = Field(
        5,
        description=(
            "Number of recent plays to show in summary. " "Default: 5. " "Optional"
        ),
        ge=1,
        le=50,
    )
...
class GetShotChartInput(BaseModel):
    ...
    entity_type: EntityType = Field(
        "player", description=("Entity type: 'player' (default) or 'team'. " "Optional")
    )
    ...
    season_type: SeasonType = Field(
        "Regular Season",
        description=(
            "Season type: 'Regular Season' (default) or 'Playoffs'. " "Optional"
        ),
    )
...
class GetClutchStatsInput(BaseModel):
    ...
    per_mode: PerModeType = Field(
        "PerGame",
        description=("Statistical mode: 'PerGame' (default) or 'Totals'. " "Optional"),
    )
...
class ListEndpointsInput(BaseModel):
    ...
    class Config:
        json_schema_extra = {"examples": [{}, {"category": "player_stats"}]}
        ...
        json_schema_extra = {
            "examples": [
                ...
                },
                {
                    ...
                    "params": {"season": "2023-24", "conference": "East"},
                },
                {
                    "endpoint": "league_leaders",
                    "params": {"stat_category": "PTS", "season": "2023-24"},
                },
            ]
        }
    ...
    class Config:
        json_schema_extra = {
            "examples": [
                {
                    "endpoint": "shot_chart",
                    "params": {"entity_name": "Stephen Curry", "season": "2023-24"},
                    "chunk_strategy": "date",
                },
                {
                    "endpoint": "play_by_play",
                    "params": {"game_date": "2024-03-15"},
                    "chunk_strategy": "game",
                },
                {

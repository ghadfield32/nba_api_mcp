...
class NBAApiClient:
    ...
    async def get_live_scoreboard(
        ...
        try:
            ...
            logger.debug(
                f"[DEBUG] get_live_scoreboard: Received target_date={target_date}, converted to date_str={date_str}"
            )
            ...
            logger.debug(
                f"[DEBUG] get_live_scoreboard: Received payload with date={payload.get('date')}, games count={len(payload.get('games', []))}"
            )
    ...
    async def get_player_game_log(
        ...
        try:
            ...
            if season is None:
                ...
                from nba_api_mcp.api.season_context import get_current_season
            ...
            logger.debug(
                f"Fetching game log for player_id={player_id}, season={season}"
            )
            ...
            if df.empty:
                return {
                    "error": f"No games found for {player_name} in {season} {season_type}"
                }
    ...
    async def get_box_score(
        ...
        try:
            ...
            box_score = await asyncio.to_thread(BoxScoreTraditionalV2, game_id=game_id)
    ...
    async def get_clutch_stats(
        ...
        try:
            ...
            logger.debug(
                f"Fetching clutch stats for {entity_name} ({entity_type}), season={season}"
            )
            ...
            if entity_type == "player":
                ...
                if df.empty:
                    return {
                        "error": f"No clutch stats found for {entity_name} in {season}"
                    }
                ...
                return {
                    "error": f"Invalid entity_type: {entity_type}. Must be 'player' or 'team'"
                }
    ...
    async def get_player_head_to_head(
        ...
        try:
            ...
            logger.debug(
                f"Fetching head-to-head: {player1_name} vs {player2_name}, season={season}"
            )
            ...
            all_players = players.get_players()
            player1_full = next(
                (p["full_name"] for p in all_players if p["id"] == player1_id),
                player1_name,
            )
            player2_full = next(
                (p["full_name"] for p in all_players if p["id"] == player2_id),
                player2_name,
            )
            ...
            player1_matchup_games = player1_games[
                player1_games["Game_ID"].isin(common_game_ids)
            ]
            player2_matchup_games = player2_games[
                player2_games["Game_ID"].isin(common_game_ids)
            ]

            # Sort by date
            player1_matchup_games = player1_matchup_games.sort_values(
                "GAME_DATE", ascending=False
            )
            player2_matchup_games = player2_matchup_games.sort_values(
                "GAME_DATE", ascending=False
            )

            # Calculate records
            player1_wins = len(
                player1_matchup_games[player1_matchup_games["WL"] == "W"]
            )
            player1_losses = len(
                player1_matchup_games[player1_matchup_games["WL"] == "L"]
            )

            player2_wins = len(
                player2_matchup_games[player2_matchup_games["WL"] == "W"]
            )
            player2_losses = len(
                player2_matchup_games[player2_matchup_games["WL"] == "L"]
            )
    ...
    async def get_player_performance_splits(
        ...
        try:
            ...
            logger.debug(
                f"Fetching performance splits: {player_name}, season={season}, last_n={last_n_games}"
            )
            ...
            def calc_averages(df: pd.DataFrame) -> Dict[str, float]:
                ...
                return {
                    ...
                    "mpg": df["MIN"].mean(),
                    "plus_minus": (
                        df["PLUS_MINUS"].mean() if "PLUS_MINUS" in df.columns else 0.0
                    ),
                }
            ...
            home_games = game_log[
                game_log["MATCHUP"].str.contains("vs\\.", na=False, regex=True)
            ]
            away_games = game_log[
                game_log["MATCHUP"].str.contains("@", na=False, regex=False)
            ]
            ...
            if last_n_stats and season_stats:
                trends = {
                    ...
                    "is_hot_streak": last_n_stats["ppg"]
                    > season_stats["ppg"] * 1.1,  # >10% above average
                    "is_cold_streak": last_n_stats["ppg"]
                    < season_stats["ppg"] * 0.9,  # >10% below average
                }
    ...
    def get_award_winners(
        ...
        if award_type not in valid_awards:
            ...
            raise ValueError(
                f"Invalid award type '{award_type}'. " f"Available awards: {available}"
            )
    ...
    async def get_player_awards(
        self, player_name: str, award_filter: Optional[str] = None
    ) -> pd.DataFrame:
        """
            ...
            if award_filter and len(awards_df) > 0:
                awards_df = awards_df[
                    awards_df["DESCRIPTION"].str.contains(
                        award_filter, case=False, na=False
                    )
                ]

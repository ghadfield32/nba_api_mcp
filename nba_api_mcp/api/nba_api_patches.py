...
def patch_scoreboardv2():
    ...
    try:
        ...
        def patched_load_response(self):
            ...
            self.game_header = scoreboardv2.Endpoint.DataSet(
                data=data_sets["GameHeader"]
            )
            self.last_meeting = scoreboardv2.Endpoint.DataSet(
                data=data_sets["LastMeeting"]
            )
            ...
            self.team_leaders = scoreboardv2.Endpoint.DataSet(
                data=data_sets["TeamLeaders"]
            )
            self.ticket_links = scoreboardv2.Endpoint.DataSet(
                data=data_sets["TicketLinks"]
            )
        ...
        logger.info(
            "✓ Applied patch: ScoreboardV2.load_response (WinProbability optional)"
        )
        return True

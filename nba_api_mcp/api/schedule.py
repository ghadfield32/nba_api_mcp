...
def parse_schedule_to_dataframe(
    ...
    for gd in game_dates:
        ...
        for g in games:
            ...
            if game_date_utc_str:
                try:
                    game_dt = datetime.fromisoformat(
                        game_date_utc_str.replace("Z", "+00:00")
                    )
            ...
            if national_tv:
                national_tv_str = ", ".join(
                    [
                        b.get("broadcastDisplay", "")
                        for b in national_tv
                        if b.get("broadcastDisplay")
                    ]
                )
            else:
...
async def get_nba_schedule(
    ...
    if season is None:
        ...
        logger.info(
            f"Auto-detected current season: {season_year-1}-{str(season_year)[2:]}"
        )
        ...
        if "-" in season:
            ...
            if len(parts) == 2:
                try:
                    ...
                    raise ValueError(
                        f"Invalid season format: {season}. Expected 'YYYY-YY' (e.g., '2025-26')"
                    )
            else:
                raise ValueError(
                    f"Invalid season format: {season}. Expected 'YYYY-YY' (e.g., '2025-26')"
        ...
        raise ValueError(
            f"Invalid season type: {type(season)}. Expected int, str, or None"
        )
...
def format_schedule_markdown(df: pd.DataFrame, max_games: int = 100) -> str:
    ...
    # Build header
    season_year = (
        df["season_year"].iloc[0]
        if "season_year" in df.columns and not df.empty
        else "Unknown"
    )
    season_stage_id = (
        df["season_stage_id"].iloc[0]
        if "season_stage_id" in df.columns and not df.empty
        else None
    )
    ...
    lines = [header, ""]
    lines.append(
        f"{'Date':<12} {'Matchup':<24} {'Time (ET)':<12} {'Arena':<25} {'Status':<12}"
    )
    lines.append("-" * 90)
    ...
    for _, row in df_display.iterrows():
        ...
        lines.append(
            f"{game_date:<12} {matchup:<24} {game_time:<12} {str(arena):<25} {str(status):<12}"
        )

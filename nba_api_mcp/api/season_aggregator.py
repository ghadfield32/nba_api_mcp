...
def _aggregate_percentage_stat(
    made_series: pd.Series, attempted_series: pd.Series
) -> float:
    """
...
def _aggregate_weighted_mean_stat(
    stat_series: pd.Series, weight_series: pd.Series
) -> float:
    """
...
class SeasonAggregator:
    ...
    async def aggregate_player_season(
        ...
        **additional_filters,
        ...
        if game_logs.empty:
            logger.warning(
                f"No game logs found for season={season}, player_id={player_id}, team_id={team_id}"
            )
            return [] if player_id is None else None

        # Determine grouping level
        grouping_level = (
            "player/team/season" if (player_id and team_id) else "player/season"
        )
        ...
        if player_id is None:
            ...
            for pid, player_games in game_logs.groupby("PLAYER_ID"):
                season_stats = self._aggregate_games(
                    player_games, season, grouping_level=grouping_level
                )
                results.append(season_stats)
            return results
        else:
            # Aggregate single player
            return self._aggregate_games(
                game_logs, season, grouping_level=grouping_level
            )

    async def aggregate_team_season(
        self, season: str, team_id: Optional[int] = None, **additional_filters
    ) -> Union[SeasonStats, List[SeasonStats]]:
        """
        ...
        if game_logs.empty:
            logger.warning(
                f"No team game logs found for season={season}, team_id={team_id}"
            )
    ...
    def _aggregate_games(
        ...
        stats = SeasonStats(season=season, grouping_level=grouping_level)
        ...
        stats.plus_minus = _aggregate_counting_stat(
            game_logs.get("PLUS_MINUS", pd.Series([0]))
        )
        ...
        stats.fg_pct = _aggregate_percentage_stat(
            game_logs.get("FGM", pd.Series([0])), game_logs.get("FGA", pd.Series([0]))
        )
        stats.fg3_pct = _aggregate_percentage_stat(
            game_logs.get("FG3M", pd.Series([0])), game_logs.get("FG3A", pd.Series([0]))
        )
        stats.ft_pct = _aggregate_percentage_stat(
            game_logs.get("FTM", pd.Series([0])), game_logs.get("FTA", pd.Series([0]))
        )
        ...
        # Special stats
        stats.double_doubles = int(
            _aggregate_counting_stat(game_logs.get("DD2", pd.Series([0])))
        )
        stats.triple_doubles = int(
            _aggregate_counting_stat(game_logs.get("TD3", pd.Series([0])))
        )

        return stats

    def aggregate_to_dataframe(
        self, game_logs: pd.DataFrame, group_by: Union[str, List[str]] = "PLAYER_ID"
    ) -> pd.DataFrame:
        """
...
def _enrich_with_awards(season_stats: SeasonStats, season: str):
    ...
    if season_stats.player_id:
        try:
            ...
            logger.warning(
                f"Failed to enrich awards for player {season_stats.player_id}: {e}"
            )
...
async def get_player_season_stats(
    ...
    **filters,
    ...
    result = await aggregator.aggregate_player_season(
        season, player_id, team_id, **filters
    )
...
async def get_team_season_stats(
    season: str, team_id: Optional[int] = None, **filters
) -> Union[pd.DataFrame, Dict[str, Any]]:
    """

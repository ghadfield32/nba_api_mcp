...
async def fetch_shot_chart_data(
    ...
    try:
        ...
        if "GAME_DATE" in df.columns:
            logger.info(
                f"DEBUG: GAME_DATE column found. Sample values: {df['GAME_DATE'].head(3).tolist()}"
            )
        else:
...
async def get_shot_chart(
    ...
    **PARAMETER FLEXIBILITY:**
    ...
    if game_date is not None:
        logger.info(
            f"[get_shot_chart] Converting game_date: {game_date} → date_from={game_date}, date_to={game_date}"
        )
        date_from = game_date
        date_to = game_date
    ...
    if normalized_seasons is None:
        ...
        from nba_api_mcp.api.tools.nba_api_client import NBAApiClient
    ...
    date_range = None
    logger.info(
        f"DEBUG: Checking for GAME_DATE column. shots_df.empty={shots_df.empty}, columns={list(shots_df.columns) if not shots_df.empty else 'N/A'}"
    )
    if not shots_df.empty and "GAME_DATE" in shots_df.columns:
        ...
        logger.warning(
            f"DEBUG: Could not extract date_range. GAME_DATE column exists: {'GAME_DATE' in shots_df.columns if not shots_df.empty else 'DataFrame is empty'}"
        )

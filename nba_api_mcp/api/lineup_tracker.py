...
class Lineup:
    ...
    def __post_init__(self):
        ...
        if self.player_ids:
            ...
            self.player_ids, self.player_names = (
                zip(*paired_sorted) if paired_sorted else ([], [])
            )
    ...
    def lineup_display(self) -> str:
        ...
        last_names = [
            name.split()[-1] if " " in name else name for name in self.player_names
        ]
    ...
    def copy(self) -> Lineup:
        ...
        return Lineup(
            player_ids=self.player_ids.copy(), player_names=self.player_names.copy()
        )
...
class LineupTracker:
    ...
    def process_play_by_play(self, pbp_df: pd.DataFrame) -> pd.DataFrame:
        ...
        pbp_df = pbp_df.copy()
        pbp_df.columns = [
            col.lower() if isinstance(col, str) else col for col in pbp_df.columns
        ]
    ...
    def _initialize_team_ids(self, pbp_df: pd.DataFrame):
        ...
        logger.debug(
            f"Identified teams: home={self.home_team_id}, away={self.away_team_id}"
        )
    ...
    def _process_substitution(self, row: pd.Series):
        ...
        in_player_id = (
            int(row.get("personid", 0)) if pd.notna(row.get("personid")) else 0
        )
        ...
        if out_pattern:
            ...
            for pid, pname in zip(
                current_lineup.player_ids, current_lineup.player_names
            ):
                if (
                    out_player_name.lower() in pname.lower()
                    or pname.lower() in out_player_name.lower()
                ):
                    out_player_id = pid
                    break
...
def add_lineups_to_play_by_play(pbp_df: pd.DataFrame, game_id: str) -> pd.DataFrame:
    """
...
async def get_play_by_play_with_lineups(
    game_id: str, start_period: int = 1, end_period: int = 10
) -> pd.DataFrame:
    """

...
def load_awards_data() -> Dict[str, List[Dict]]:
    ...
    awards_file = (
        Path(__file__).parent.parent.parent / "api_documentation" / "awards_data.json"
    )
...
def get_player_awards_for_season(player_id: int, season: str) -> Dict[str, bool]:
    """
...
def format_award_winners_text(
    award_type: str, winners: List[Dict], max_display: int = 10
) -> str:
    """
    ...
    # Build header
    count_text = (
        f"Last {len(winners)}" if len(winners) <= max_display else f"Top {max_display}"
    )
    lines = [f"{display_name} Winners ({count_text} Seasons)", "=" * 50]

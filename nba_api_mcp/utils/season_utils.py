...
def parse_season_input(season: str) -> List[str]:
    ...
    if ":" in season or ".." in season:
        ...
        if len(parts) != 2:
            raise ValueError(
                f"Season range must have format 'START{separator}END', got: {season}"
            )

        start_season, end_season = parts[0].strip(), parts[1].strip()
        seasons = expand_season_range(start_season, end_season)
        logger.info(
            f"Expanded season range {start_season} to {end_season}: {len(seasons)} seasons"
        )
        return seasons
...
def expand_season_range(start_season: str, end_season: str) -> List[str]:
    ...
    if start_year > end_year:
        raise ValueError(
            f"Start season {start_season} must be before end season {end_season}"
        )
...
def validate_season_format(season: str) -> None:
    ...
    try:
        ...
        if year2 != expected_year2:
            raise ValueError(
                f"Season format error: {year1}-{year2:02d} (expected {year1}-{expected_year2:02d})"
            )
...
AVAILABLE_SEASONS = expand_season_range(
    "1996-97", CURRENT_SEASON
)  # NBA API data starts 1996-97

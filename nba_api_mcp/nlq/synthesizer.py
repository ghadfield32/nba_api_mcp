...
def extract_multi_season_results(
    execution_result: ExecutionResult, base_tool_name: str
) -> List[ToolResult]:
    """
...
def synthesize_player_stats_query(
    ...
    multi_results = extract_multi_season_results(
        execution_result, "get_player_advanced_stats"
    )

    if len(multi_results) > 1:
        # Multi-season query detected
        return synthesize_multi_season_stats(
            parsed, execution_result, "get_player_advanced_stats"
        )
...
def synthesize_team_stats_query(
    ...
    # Phase 5.2 (P2): Check for multi-season results first
    multi_results = extract_multi_season_results(
        execution_result, "get_team_advanced_stats"
    )

    if len(multi_results) > 1:
        # Multi-season query detected
        return synthesize_multi_season_stats(
            parsed, execution_result, "get_team_advanced_stats"
        )
    ...
    data = stats_result.data
    team_name = data.get(
        "team_name", parsed.entities[0]["name"] if parsed.entities else "Unknown"
    )
    season = data.get("season", "Unknown")
...
def _build_game_context_narrative(data: dict, parsed: ParsedQuery) -> str:
    ...
    if "advanced_stats" in data:
        ...
        if team1_stats or team2_stats:
            ...
            if off1 or off2:
                ...
                lines.append(
                    f"- **Offense**: {team1_name} ({off1:.1f}) vs {team2_name} ({off2:.1f}) → Advantage: {adv1}"
                )
            ...
            if def1 or def2:
                ...
                lines.append(
                    f"- **Defense**: {team1_name} ({def1:.1f}) vs {team2_name} ({def2:.1f}) → Advantage: {adv2}"
                )
            lines.append("")
    ...
    if len(lines) <= 3:
        return (
            f"Game context for {team1_name} vs {team2_name} is currently unavailable."
        )
...
def synthesize_filtered_games_query(
    ...
    try:
        ...
        data = (
            json.loads(games_result.data)
            if isinstance(games_result.data, str)
            else games_result.data
        )
...
def synthesize_multi_season_stats(
    parsed: ParsedQuery, execution_result: ExecutionResult, base_tool_name: str
) -> str:
    """
    ...
    try:
        ...
        if len(results) == 1:
            ...
            logger.warning(
                "synthesize_multi_season_stats called with single season, using fallback"
            )
        ...
        # Build header
        first_season = (
            parsed.time_range.seasons[0] if parsed.time_range.seasons else "Unknown"
        )
        last_season = (
            parsed.time_range.seasons[-1] if parsed.time_range.seasons else "Unknown"
        )
        ...
        if all(isinstance(sd.get("raw"), str) for sd in season_data):
            ...
            for i, (result, season) in enumerate(
                zip(results, parsed.time_range.seasons or [])
            ):
                answer += f"### {season}\n\n"
                answer += result.data
                if i < len(results) - 1:
            ...
            answer += (
                f"*Note: Multi-season aggregation for structured data coming soon*\n\n"
            )
    ...
    if isinstance(data, list):
        ...
        if parsed.modifiers.get("stat_filters"):
            criteria_parts.append(
                f"statistical filters: {parsed.modifiers['stat_filters']}"
            )
        if parsed.modifiers.get("min_games"):
            criteria_parts.append(
                f"minimum {parsed.modifiers['min_games']} games played"
            )

        criteria_str = (
            " and ".join(criteria_parts) if criteria_parts else "specified criteria"
        )
...
def synthesize_lineup_comparison_query(
    ...
    try:
        ...
        from nba_api_mcp.nlq.planner import _extract_season

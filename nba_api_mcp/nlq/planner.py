...
ANSWER_PACK_TEMPLATES = {
        ...
        "tools": lambda parsed: [
            ToolCall(
                ...
                params={
                    ...
                    "limit": parsed.modifiers.get("worst_n")
                    or parsed.modifiers.get("top_n", 10),  # Phase 5.3: worst_n support
                    "min_games_played": parsed.modifiers.get(
                        "min_games"
                    ),  # Phase 5.3: min_games support
                },
                parallel_group=0,
            )
        ],
    ...
    "player_stats": {
        ...
        "tools": lambda parsed: _build_multi_season_player_stats(
            parsed
        ),  # Phase 5.2 (P2): Multi-season support
    },
    ...
    "team_stats": {
        ...
        "optional": ["time_range"],
        "tools": lambda parsed: _build_multi_season_team_stats(
            parsed
        ),  # Phase 5.2 (P2): Multi-season support
    },
            ...
            ToolCall(
                ...
                params={
                    ...
                    "team": (
                        ...
                        if parsed.entities
                        and parsed.entities[0]["entity_type"] == "team"
                        else None
                    ),
    ...
    "player_game_stats": {
        ...
        "tools": lambda parsed: _build_multi_season_player_game_stats(
            parsed
        ),  # Phase 5.2 (P2): Multi-season support (includes last_n_games)
    },
    ...
    "season_stats_aggregate": {
        ...
        "required": ["entities"],  # 1 player or team
        "optional": ["time_range"],
        "tools": lambda parsed: _build_multi_season_season_stats(
            parsed
        ),  # Phase 5.2 (P2): Multi-season support
    },
        ...
        "tools": lambda parsed: [
            ToolCall(
                ...
                params={
                    ...
                    "per_mode": _extract_per_mode(parsed.modifiers),
                    "limit": parsed.modifiers.get("worst_n")
                    or parsed.modifiers.get("top_n", 25),  # Phase 5.3: worst_n support
    ...
    "lineup_analysis": {
        ...
        "optional": [
            "time_range",
            "modifiers.min_minutes",
            "modifiers.lineup_type",
            "modifiers.with_player",
            "modifiers.without_player",
        ],
        "tools": lambda parsed: [
            ...
            ToolCall(
                ...
                params=(
                    # Multi-season parameters
                    {
                        ...
                        "seasons": str(
                            parsed.time_range
                        ),  # Will be parsed by get_lineup_stats_multi_season
        ...
        "tools": lambda parsed: [
            ToolCall(
                tool_name=(
                    ...
                    and parsed.entities
                    and parsed.entities[0]["entity_type"] == "player"
...
def match_template(parsed: ParsedQuery) -> Optional[str]:
    ...
    if intent == "comparison":
        ...
        if num_entities == 2 and all(t == "team" for t in entity_types):
            if any(
                keyword in parsed.raw_query.lower()
                for keyword in ["five-man", "5-man", "rotation"]
            ):
                return "lineup_comparison"
...
async def generate_execution_plan(parsed: ParsedQuery) -> Optional[ExecutionPlan]:
    ...
    if not template_name:
        ...
        if tool_calls:
            logger.info(f"LLM plan generation successful: {len(tool_calls)} tool calls")
            ...
            logger.error(
                "No template matched and LLM plan generation failed - cannot generate plan"
            )
            return None
    ...
    if validation_warnings:
        logger.warning(
            f"Template validation warnings for '{template_name}': {validation_warnings}"
        )
        ...
        for warning in validation_warnings:
            if "Missing required field: entities" in warning:
                parsed.suggestions.append(
                    "Try adding a player or team name to your query"
                )
            elif "Missing required field: stat_types" in warning:
                parsed.suggestions.append(
                    "Try specifying a statistic (e.g., 'points', 'assists', 'rebounds')"
                )

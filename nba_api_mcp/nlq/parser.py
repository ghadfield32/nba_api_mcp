...
class TimeRange:
    ...
    seasons: Optional[List[str]] = (
        None  # Phase 5.2 (P2): ["2020-21", "2021-22", "2022-23"]
    )
...
def parse_calendar_anchor(query: str, today: date) -> Optional[TimeRange]:
    ...
    for anchor_name, anchor_date in anchors.items():
        if (
            f"since {anchor_name}" in query_lower
            or f"after {anchor_name}" in query_lower
        ):
            return TimeRange(
...
def extract_modifiers(query: str) -> Dict[str, Any]:
    ...
    opponent_match = re.search(
        r"(?:vs|versus|against)\s+([A-Za-z]+(?:\s+[A-Za-z]+)?)", query_lower
    )
    if opponent_match:
        modifiers["opponent"] = opponent_match.group(1).strip()

    # Win/Loss outcome (Phase 2.4)
    if re.search(r"\bwins?\b|\bwon\b", query_lower) and not re.search(
        r"who will win", query_lower
    ):
        modifiers["outcome"] = "W"
    ...
    if (
        "starting lineup" in query_lower
        or "starting five" in query_lower
        or "starting unit" in query_lower
    ):
        modifiers["lineup_type"] = "starting"
    elif (
        "bench lineup" in query_lower
        or "bench unit" in query_lower
        or "second unit" in query_lower
    ):
        modifiers["lineup_type"] = "bench"

    # with_player (lineups including player)
    with_player_match = re.search(
        r"(?:with|including)\s+([A-Za-z\s]+?)(?:\s+lineup|\s+and|\s+,|$)", query_lower
    )
    ...
    without_player_match = re.search(
        r"(?:without|excluding|except)\s+([A-Za-z\s]+?)(?:\s+lineup|\s+and|\s+,|$)",
        query_lower,
    )
    ...
    min_games_match = re.search(
        r"(?:min(?:imum)?|at least)\s+(\d+)\s+games?", query_lower
    )
    if min_games_match:
    ...
    if re.search(
        r"\bclutch\b|\bcrunch time\b|final \d+ minutes?|close games?", query_lower
    ):
    ...
    for month_name, month_num in months.items():
        if re.search(rf"\b(?:in|during)\s+{month_name}\b", query_lower) or re.search(
            rf"\b{month_name}\s+games?\b", query_lower
        ):
...
def generate_validation_feedback(parsed: ParsedQuery) -> None:
    ...
    if parsed.intent == "unknown":
        ...
        suggestions.append(
            "Try queries like: 'Who leads in points?', 'Lakers vs Celtics', 'LeBron stats this season'"
        )

    # Issue 2: Low confidence (< 0.7)
    if parsed.confidence < 0.7:
        issues.append(f"Low parsing confidence ({parsed.confidence:.0%})")
        if not parsed.entities and parsed.intent in [
            "comparison",
            "player_stats",
            "team_stats",
        ]:
            suggestions.append(
                "Try including specific player or team names (e.g., 'LeBron James', 'Lakers')"
            )
    ...
    if parsed.intent == "comparison":
        if len(parsed.entities) < 2:
            ...
            suggestions.append(
                "Try: 'Compare LeBron James and Kevin Durant' or 'Lakers vs Celtics'"
            )
        elif len(parsed.entities) > 2:
            issues.append(
                f"Too many entities for comparison ({len(parsed.entities)} found, expected 2)"
            )
    ...
    if parsed.intent == "leaders" and not parsed.stat_types:
        ...
        suggestions.append(
            "Try: 'Who leads in points?', 'Top 10 assists', 'Best rebounders'"
        )

    # Issue 5: Entity-based query without entities
    if (
        parsed.intent in ["player_stats", "team_stats", "game_context"]
        ...
        suggestions.append(
            f"Try adding a {entity_type} name (e.g., 'LeBron James', 'Lakers', 'Warriors')"
        )

    # Issue 6: Low entity confidence
    if parsed.entities:
        low_conf_entities = [
            e for e in parsed.entities if e.get("confidence", 1.0) < 0.8
        ]
        if low_conf_entities:
            ...
            suggestions.append(
                "Try using full names or common abbreviations (e.g., 'LAL' for Lakers)"
            )

    # Issue 7: Ambiguous stat types (multiple possible interpretations)
    if len(parsed.stat_types) > 5:
        issues.append(
            f"Query matched many stat types ({len(parsed.stat_types)}), results may be unfocused"
        )
        suggestions.append(
            "Try being more specific (e.g., 'points' instead of 'scoring stats')"
        )
...
async def parse_query(query: str) -> ParsedQuery:
    ...
    if confidence < 0.5:
        ...
        logger.info(f"Low confidence ({confidence:.2f}), attempting LLM refinement...")
        ...
        if refined_parse.confidence > confidence:
            ...
            logger.debug(
                "LLM refinement did not improve confidence, using original parse"
            )
...
def validate_parsed_query(parsed: ParsedQuery) -> ValidationResult:
    ...
    if parsed.confidence < 0.3:
        ...
        result.hints.append(
            "Example: 'LeBron James points this season' instead of 'show stats'"
        )
    ...
    if parsed.intent == "comparison":
        if len(parsed.entities) < 2:
            ...
            result.errors.append(
                "Comparison queries require at least 2 players or teams"
            )
            result.hints.append("Example: 'Compare LeBron James and Kevin Durant'")
            result.hints.append("Example: 'Lakers vs Celtics'")
        elif len(parsed.entities) > 2:
            result.warnings.append(
                f"Found {len(parsed.entities)} entities, using first 2 for comparison"
            )
    ...
    if parsed.intent in ["player_stats", "team_stats"]:
        if len(parsed.entities) < 1:
            result.valid = False
            result.errors.append(
                f"'{parsed.intent}' queries require at least 1 player or team name"
            )
    ...
    if parsed.intent == "leaders":
        if not parsed.stat_types:
            result.warnings.append(
                "No specific stat mentioned, will use default (points)"
            )
    ...
    if parsed.intent == "awards":
        if not any(
            kw in parsed.raw_query.lower()
            for kw in ["mvp", "dpoy", "roy", "all-nba", "all-star"]
        ):
    ...
    if parsed.intent == "streaks":
        if not parsed.entities:
            result.warnings.append(
                "Streak queries work best with a specific team or player"
            )
    ...
    if parsed.intent == "milestones":
        if not parsed.entities:
            result.warnings.append(
                "Milestone queries usually need a player or team name"
            )

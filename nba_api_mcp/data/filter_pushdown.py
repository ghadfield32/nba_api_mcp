...
class FilterPushdownMapper:
    ...
    def can_push_filter(self, endpoint: str, column: str, operator: str) -> bool:
        """
    ...
    def convert_filter_to_param(
        self, endpoint: str, column: str, operator: str, value: Any
    ...
    def split_filters(
        ...
        for column, filter_spec in filters.items():
            ...
            if result:
                ...
                logger.debug(
                    f"Pushed filter to API: {column} {operator} {value} → {api_param}={api_value}"
                )
                ...
                post_filters[column] = filter_spec
                logger.debug(
                    f"Cannot push filter, applying post-fetch: {column} {operator} {value}"
                )

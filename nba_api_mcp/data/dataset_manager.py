...
def get_default_save_path(
    endpoint: str = "dataset", format: str = "parquet", base_dir: str = "mcp_data"
) -> Path:
    """
...
def generate_descriptive_filename(data: Dict[str, Any]) -> str:
    ...
    # 3. Player Advanced Stats
        ...
        player_name = actual_data.get("player_name") or actual_data.get(
            "player", {}
        ).get("name", "unknown")
    ...
    # 4. Team Stats
        ...
        team_name = actual_data.get("team_name") or actual_data.get("team", {}).get(
            "name", "unknown"
        )
...
def save_parquet(
    df: pd.DataFrame, filename_base: str, base_dir: str = "mcp_data"
) -> Path:
    """
...
def save_text(text: str, filename_base: str, base_dir: str = "mcp_data") -> Path:
    """
...
class DatasetManager:
    ...
    async def list_handles(self, include_expired: bool = False) -> List[DatasetHandle]:
        """

...
Usage:
    from nba_api_mcp.data.endpoint_registry import register_endpoint, get_registry
...
class EndpointRegistration:
    ...
    def __post_init__(self):
        ...
        if not asyncio.iscoroutinefunction(self.handler):
            raise ValueError(f"Handler for '{self.name}' must be an async function")
...
class EndpointRegistry:
    ...
    def register(
        ...
        if name in self._endpoints:
            logger.warning(f"Endpoint '{name}' is already registered. Overwriting...")
    ...
    def list_endpoints(
        self, tag: Optional[str] = None, supports_batch: Optional[bool] = None
    ) -> List[str]:
        """
        ...
        if tag is not None:
            endpoints = [
                name for name in endpoints if tag in self._endpoints[name].tags
            ]
    ...
    def validate_all(self) -> Dict[str, List[str]]:
        ...
        }  # Only return endpoints with errors
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "tags": {tag: len(names) for tag, names in self._tags.items()},
        }

...
from nba_api_mcp.data.fetch import FetchError, fetch_endpoint
from nba_api_mcp.data.introspection import EndpointCapabilities, get_introspector
...
class DatasetPaginator:
    ...
    async def fetch_chunked(
        ...
        """
        logger.info(
            f"Starting chunked fetch for {endpoint} with strategy={chunk_strategy}"
        )
    ...
    async def fetch_with_progress(
        ...
        async for table, chunk_info in self.fetch_chunked(
            ...
            params,
            chunk_strategy,
            check_size_limit=False,  # Already checked above
        ):
            chunks.append(table)
        ...
        logger.info(
            f"Completed fetch: {result.num_rows} rows, {result.num_columns} columns"
        )
        return result
    ...
    async def _fetch_by_date(
        ...
        logger.info(
            f"Date-based chunking: {total_chunks} chunks from {date_from} to {date_to}"
        )
        ...
        for chunk_num, (chunk_start, chunk_end) in enumerate(chunks, start=1):
            ...
            logger.debug(
                f"Fetching chunk {chunk_num}/{total_chunks}: {chunk_start} to {chunk_end}"
            )

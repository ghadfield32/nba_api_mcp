...
from nba_api_mcp.api.data_groupings import GranularityLevel, GroupingLevel
from nba_api_mcp.data.joins import JoinError, join_tables, validate_join_columns
...
class MergeManager:
    ...
    def __init__(
        self, validation_level: MergeValidationLevel = MergeValidationLevel.WARN
    ):
        """
        ...
        if metrics is None or "TRUE_SHOOTING_PCT" in metrics:
            if all(c in df.columns for c in ["PTS", "FGA", "FTA"]):
                metrics_df["TRUE_SHOOTING_PCT"] = df.apply(
                    lambda row: calculate_true_shooting_pct(
        ...
        if metrics is None or "EFFECTIVE_FG_PCT" in metrics:
            if all(c in df.columns for c in ["FGM", "FG3M", "FGA"]):
                metrics_df["EFFECTIVE_FG_PCT"] = df.apply(
                    lambda row: calculate_effective_fg_pct(
                        row["FGM"], row["FG3M"], row["FGA"]
                    ),
                    axis=1,
                )
    ...
    def merge_shot_chart_data(
        ...
        if aggregation == "count":
            agg_shots = (
                shots_df.groupby(config.identifier_columns)
                .size()
            ...
            for zone in ["Paint", "Mid-Range", "Three Point"]:
                ...
                if not zone_df.empty:
                    zone_agg = (
                        zone_df.groupby(config.identifier_columns)
                        .agg({"SHOT_MADE_FLAG": ["sum", "count", "mean"]})
                        .reset_index()
                    )
            ...
            for zone_df in zone_stats[1:]:
                agg_shots = pd.merge(
                    agg_shots, zone_df, on=config.identifier_columns, how="outer"
                )
    ...
    def _validate_pre_merge(
        ...
        base_duplicates = base_df.duplicated(
            subset=identifier_columns, keep=False
        ).sum()
        ...
        merge_duplicates = merge_df.duplicated(
            subset=identifier_columns, keep=False
        ).sum()
    ...
    def _handle_validation_result(
        self, validation_result: MergeValidationResult, stage: str
    ):

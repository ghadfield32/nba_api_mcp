...
class DatasetInfo:
    ...
    def to_markdown(self) -> str:
        ...
        for col in self.all_columns:
            if (
                col.name not in self.identifier_columns
                and col.name not in self.optional_identifier_columns
                and col.name not in self.special_columns
            ):
                md += f"- **{col.name}** ({col.data_type}): {col.description}\n"
...
class DatasetInspector:
    ...
    def _get_merge_compatibility(self, level: GroupingLevel) -> List[str]:
        ...
        same_gran = [
            l.value
            for l in GroupingLevel
            if l in MERGE_CONFIG_CATALOG
            and MERGE_CONFIG_CATALOG[l].granularity
            == MERGE_CONFIG_CATALOG[level].granularity
        ]
    ...
    def search_datasets(self, search_term: str) -> List[str]:
        ...
        for level_name, info in self.datasets.items():
            ...
            for col in info.all_columns:
                if (
                    search_lower in col.name.lower()
                    or search_lower in col.description.lower()
                ):
                    results.append(level_name)
                    break

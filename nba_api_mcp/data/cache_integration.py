...
from nba_api_mcp.cache.redis_cache import CacheTier, LRUCache, RedisCache
...
class CacheManager:
    ...
    def enable_parquet_cache(
        ...
        logger.info(
            f"✅ Parquet cache enabled at {cache_dir} (max: {max_size_mb}MB, compression: {compression})"
        )
    ...
    async def get_or_fetch(
        ...
        try:
            ...
            if cached_data is not None:
                self.stats["hits"] += 1
                logger.debug(
                    f"Cache HIT (Tier 1/2) for {endpoint} (key: {cache_key[:20]}...)"
                )
        ...
        if self._parquet_enabled and self.parquet_backend:
            try:
                ...
                if parquet_data is not None:
                    self.stats["hits"] += 1
                    logger.info(
                        f"✅ Cache HIT (Tier 3 Parquet) for {endpoint} - loaded {len(parquet_data)} rows from persistent cache"
                    )
        ...
        self.stats["misses"] += 1
        logger.debug(
            f"Cache MISS (all tiers) for {endpoint} (key: {cache_key[:20]}...)"
        )

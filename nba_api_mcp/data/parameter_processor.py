...
from nba_api_mcp.api.errors import EntityNotFoundError
from nba_api_mcp.data.catalog import ParameterSchema, get_catalog
...
class ParameterProcessor:
    ...
    async def process(
        ...
        transformations.extend(
            self._get_alias_transformations(params, normalized_params)
        )
        ...
        for param_name, param_value in normalized_params.items():
            ...
            param_schema = next(
                (p for p in endpoint_meta.parameters if p.name == param_name), None
            )

            if param_schema is None:
                # Unknown parameter - log warning but include it
                warnings.append(
                    f"Unknown parameter '{param_name}' for endpoint '{endpoint}'"
                )
            ...
            try:
                processed_value = self._process_parameter_value(
                    param_name, param_value, param_schema, transformations
                )
        ...
        if resolve_entities:
            await self._resolve_entities(
                processed_params, resolved_entities, transformations
            )
    ...
    def _get_alias_transformations(
        self, original: Dict[str, Any], normalized: Dict[str, Any]
    ) -> List[str]:
    ...
    def _parse_date(self, value: Any) -> str:
        ...
        if not isinstance(value, str):
            raise ValueError(
                f"Date must be string, date, or datetime, got {type(value)}"
            )
    ...
    def _apply_smart_defaults(self, params: Dict[str, Any], transformations: List[str]):
        """

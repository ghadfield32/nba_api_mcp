...
Integration with unified_fetch:
    from nba_api_mcp.data.query_optimizer import QueryOptimizer
...
class CostEstimator:
    ...
    def estimate_join_cost(
        self, left_rows: int, right_rows: int, join_type: str = "inner"
    ) -> Tuple[float, int]:
        """
...
class QueryOptimizer:
    ...
    def optimize(self, operations: List[Dict[str, Any]]) -> ExecutionPlan:
        ...
        plan = ExecutionPlan(
            operations=query_ops, optimization_applied=optimizations_applied
        )
    ...
    def _parse_operations(
        ...
        for op_dict in operations:
            ...
            query_op = QueryOperation(op_type=op_type, params=op_dict)
    ...
    def _push_filters_before_joins(
        ...
        join_positions = [
            i for i, op in enumerate(result) if op.op_type == OperationType.JOIN
        ]
    ...
    def _merge_filter_operations(self, filters: List[QueryOperation]) -> QueryOperation:
        ...
        return QueryOperation(
            op_type=OperationType.FILTER, params={"filters": combined_filters}
        )
    ...
    def _update_costs(self, operations: List[QueryOperation]) -> None:
        ...
        for op in operations:
            if op.op_type == OperationType.FETCH:
                ...
                cost, rows = self.cost_estimator.estimate_filter_cost(
                    current_rows, num_filters
                )
                op.estimated_cost = cost
                op.estimated_rows = rows
                current_rows = rows

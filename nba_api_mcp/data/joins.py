...
def validate_join_columns(
    ...
    if isinstance(on, str):
        ...
        raise JoinError(
            f"Invalid 'on' parameter type: {type(on)}. " f"Must be str, list, or dict."
        )
...
def anti_join(left: pa.Table, right: pa.Table, on: Union[str, List[str]]) -> pa.Table:
    ...
    if isinstance(on, str):
        ...
        on_clause = " AND ".join(
            [f"left_table.{col} = right_table.{col}" for col in on]
        )
    else:
...
def filter_table(table: pa.Table, conditions: List[Dict[str, Any]]) -> pa.Table:
    """
    ...
    for cond in conditions:
        ...
        if isinstance(value, str):
            ...
            # Build list items without nested f-strings
            list_items = [f"'{v}'" if isinstance(v, str) else str(v) for v in value]

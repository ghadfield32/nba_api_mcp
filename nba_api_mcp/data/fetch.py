...
from nba_api_mcp.api.errors import EntityNotFoundError, NBAApiError
from nba_api_mcp.api.tools.leaguegamelog_tools import fetch_league_game_log
from nba_api_mcp.api.tools.playercareerstats_leagueleaders_tools import (
    get_league_leaders,
    get_player_career_stats,
)
from nba_api_mcp.data.catalog import get_catalog
from nba_api_mcp.data.dataset_manager import ProvenanceInfo
from nba_api_mcp.data.endpoint_registry import get_registry, register_endpoint
    ...
    try:
        ...
        if granularity == "raw":
            ...
            summary = {
                ...
                "THREE_PT_ATTEMPTS": len(
                    shot_df[shot_df["SHOT_TYPE"] == "3PT Field Goal"]
                ),
...
async def _fetch_player_head_to_head(
    ...
    try:
        ...
        if combined_stats.empty:
            logger.warning(
                f"No head-to-head matchups found for {player1_name} vs {player2_name}"
            )
    ...
    try:
        ...
        if game_date and team:
            ...
            if pbp_instance and pbp_instance.game_id:
                ...
                if df.empty:
                    logger.warning(
                        f"No play-by-play data found for {team} on {game_date}"
                    )
...
async def _fetch_league_player_games(
    ...
    try:
        ...
        # Fetch from NBA API
        result = await asyncio.to_thread(PlayerGameLogs, **api_params)

...
from nba_api_mcp.data.parameter_processor import ParameterValidationError, get_processor
...
async def unified_fetch(
    ...
    if filters:
        ...
        api_filter_params, post_fetch_filters = pushdown_mapper.split_filters(
            endpoint=endpoint, filters=filters, params=processed.params
        )
    ...
    try:
        if use_cache:
            data, from_cache = await cache_mgr.get_or_fetch(
                endpoint, processed.params, fetch_func, force_refresh=force_refresh
            )
        ...
        if table.num_rows == 0:
            ...
            try:
                ...
                provenance.operations.append(
                    f"post_filter:{len(post_fetch_filters)} conditions"
                )
                ...
                processed.warnings.append(
                    f"Post-fetch filter application failed: {str(e)}"
                )
...
async def batch_fetch(
    requests: List[Dict[str, Any]], as_arrow: bool = True, max_concurrent: int = 5
...
def apply_filters(table: pa.Table, filters: Dict[str, List[Any]]) -> pa.Table:
    """
